...
4. For a new onboarding, run `scripts/start_country.py` to create the source inventory, intake report, and schema-v5 manifest in one validated step. The script must start from clean repository baselines and refuses to replace an existing onboarding record.
...
Use `scripts/check_manifest.py init` only when reconstructing a manifest around an already-reviewed inventory. Upgrade an existing schema-v4 record with `scripts/check_manifest.py upgrade --manifest <v4.json> --output <v5.json>`; the command never overwrites the old file. After every work session, run `scripts/check_manifest.py next --manifest <onboarding-manifest.json>` and leave its first incomplete gate with an executable `next_action`.
...
| Workbook | Inventory every sheet and audit formula caches, duplicate names, headers, rows, and business keys | workbook inventory, duplicate contract, duplicate report | an ingested sheet is unaccounted for, a formula cache is missing, or a duplicate lacks disposition |
| Subnational scope | Decide central-only versus subnational; choose the target level and required datasets | reviewed decision or scope report | geographic meaning, target level, or required inputs are unresolved |
| Classification | Extract formulas and overrides; test overcounting and foreign-funding logic | rule table, parser coverage, overlap report, ownership ledger, foreign report | a published formula lacks independent support or ownership is undecided |
...
| Subnational data | Align geography, boundaries, population, and required indicators | decision report or admin contract and coverage report | mapping, boundary, geometry, population, or no-data coverage is unresolved |
...
- `$mega-subnational-onboarding` for the scope decision during intake and the data-readiness audit after country ETL;
...
- Keep `admin*` spender fields separate from `geo*` allocation fields. Do not let full boundary or indicator preparation delay unrelated workbook analysis after the scope and target level are settled.
...
Run it with `--ready` only for release. Every standard gate must be `passed`; a central-only country passes both subnational gates with its reviewed decision report rather than `not_applicable`.
...
3. duplicate, parser, overcounting, foreign-funding, discrepancy, subnational-scope, and subnational-data results;
